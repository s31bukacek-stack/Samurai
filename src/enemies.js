...
export function createEnemies(sprite) {
  ...
  return {
    ...
    update(dtMs, player) {
      ...
      for (const e of list) {
        if (e.knocked) {
          e.vy += cfg.knockGravity * dt;   // jen mírná gravitace → čistá linka
          e.wx += e.vx * dt;
          e.wy += e.vy * dt;
          e.rot += e.vrot * dt;
          e.life -= dt;
        ...
        // Sek zasáhne jen dušíky VE SMĚRU míření (ne ty na opačné straně)
        // a odpálí je přesně ve zvoleném úhlu.
        if (player.isAttacking) {
          ...
          if (rd < cfg.hitRadius) {
            ...
            // je dušík ve směru seku? (jinak sek dolů nesmí trefit ty nad tebou)
            if (nx * player.attackAimX + ny * player.attackAimY > cfg.hitDot) {
              e.knocked = true;
